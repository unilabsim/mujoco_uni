...
void _unsafe_batch_forward_range(std::vector<const mjModel*>& m, mjData* d, int start_batch,
                                 int end_batch, const mjtNum* state0,
...
void _unsafe_batch_forward_threaded(std::vector<const mjModel*>& m, std::vector<mjData*>& d,
                                    int nbatch, const mjtNum* state0,
                                    const mjtNum* warmstart0, mjtNum* state,
                                    mjtNum* sensordata, int skipsensor,
  ...
  for (int j = 0; j < nfulljobs; j++) {
    auto task = [=, &m, &d](void) {
      ...
      _unsafe_batch_forward_range(
          m, d[id], j * chunk_size, (j + 1) * chunk_size,
          state0, warmstart0, state, sensordata, skipsensor);
    };
    pool->Schedule(task);
  }
...
class BatchForwardRunner {
 public:
  ...
  py::tuple forward(py::list m, py::list d, py::object state0,
    ...
    if (py::len(m) == 1) {
      const raw::MjModel* single_model = m[0].cast<const MjModelWrapper*>()->get();
    ...
    if (nthread_ == 0 && py::len(d) > 1) {
      throw py::value_error("More than one data instance passed but forward runner is configured to run on main thread");
    }
    ...
    mjtNum* warmstart0_ptr = get_array_ptr(warmstart0_np, "warmstart0", nbatch, nv);
    ...
    mjtNum* sensordata_ptr = get_array_ptr(sensordata_opt, "sensordata", nbatch, nsensordata);
...
PYBIND11_MODULE(_batch_forward, pymodule) {
  py::class_<BatchForwardRunner>(pymodule, "BatchForwardRunner")
      .def(py::init([](int nthread) { return std::make_unique<BatchForwardRunner>(nthread); }),
           py::kw_only(), py::arg("nthread"))
      .def("forward", &BatchForwardRunner::forward,
           py::arg("model"), py::arg("data"), py::arg("state0"),
