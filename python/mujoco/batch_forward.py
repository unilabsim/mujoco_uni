...
class BatchForwardRunner:
    ...
    def __init__(self, *, nthread: Optional[int] = None):
        ...
        self._runner = _native_batch_forward.BatchForwardRunner(nthread=self.nthread)
    ...
    def forward(
        ...
        *,
        ...
        state_np, sensor_np = self._runner.forward(
            ...
            data,
            state0,
            None if initial_warmstart is None else np.asarray(initial_warmstart, dtype=np.float64),
            chunk_size,
            bool(skipsensor),
        )
