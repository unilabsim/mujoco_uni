...
void _unsafe_rollout_threaded(std::vector<const mjModel*>& m, std::vector<mjData*>& d,
  ...
  for (int j = 0; j < nfulljobs; j++) {
    auto task = [=, &m, &d](void) {
      ...
      _unsafe_rollout(m, d[id], j * chunk_size, (j + 1) * chunk_size, nstep,
...
class RolloutMLX {
 public:
  explicit RolloutMLX(int nthread) : nthread_(nthread) {
    if (nthread_ > 0) pool_ = std::make_shared<ThreadPool>(nthread_);
  }

  py::tuple rollout(py::list m, py::list d, int nstep, unsigned int control_spec,
    ...
    if (nthread_ == 0 && py::len(d) > 1) {
      throw py::value_error("More than one data instance passed but rollout is configured to run on main thread");
    }
...
PYBIND11_MODULE(_rollout_mlx, pymodule) {
  py::class_<RolloutMLX>(pymodule, "RolloutMLX")
      .def(py::init([](int nthread) { return std::make_unique<RolloutMLX>(nthread); }),
           py::kw_only(), py::arg("nthread"))
      .def("rollout", &RolloutMLX::rollout, py::arg("model"), py::arg("data"), py::arg("nstep"),
